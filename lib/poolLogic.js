...
// True only if this week's latest lock time hasn't passed yet AND is coming
// up within the next few days -- not just "sometime, eventually." Used to
// stop the reminder cron from nagging people about a deadline that's still
// weeks away (e.g. before the season has even started).
export function hasUpcomingDeadlineSoon(gamesByWeek, week, now = new Date(), withinDays = 3) {
  const games = (gamesByWeek[week] || []).filter((g) => !ineligible(g.spread, week));
  const lockTimes = games.map((g) => computeLockTime(g)).filter(Boolean);
  if (lockTimes.length === 0) return false;
  const maxLock = Math.max(...lockTimes.map((d) => d.getTime()));
  const windowMs = withinDays * 24 * 60 * 60 * 1000;
  return maxLock > now.getTime() && maxLock - now.getTime() <= windowMs;
}
